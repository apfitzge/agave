...
mod tests {
    ...
    #[allow(deprecated)]
    fn test_get_fee_for_versioned_message_serialization(message: VersionedMessage) {
        ...
        thread::spawn(move || {
            ...
            io.add_method("getFeeForMessage", move |params: Params| match params {
                Params::Array(p) => {
                    assert_eq!(p.len(), 2);
                    assert_eq!(p[1], json!(CommitmentConfig::processed()));
        ...
        let fee = rpc_client.get_fee_for_versioned_message(&message).unwrap();
        assert_eq!(fee, 42);

        // Keep the deprecated API compatible for its supported message types.
        let legacy_fee = match &message {
            ...
            VersionedMessage::V1(message) => rpc_client.get_fee_for_message(message),
        };
        assert_eq!(legacy_fee.unwrap(), fee);
    }
