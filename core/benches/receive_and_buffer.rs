use {
    ...
    solana_core::banking_stage::{
        ...
        transaction_scheduler::{
            receive_and_buffer::{
                NoCopyTransactionViewReceiveAndBuffer, PacketBatchBytes, ReceiveAndBuffer,
                SanitizedTransactionReceiveAndBuffer, TransactionViewReceiveAndBuffer,
            },
...
fn generate_no_copy_transactions(
    num_txs: usize,
    bank: Arc<Bank>,
    fee_payers: &[Keypair],
    num_instructions_per_tx: usize,
    ...
    let mut bytes = BytesMut::with_capacity(num_txs * PACKET_DATA_SIZE);
    let txs: Vec<Bytes> = (0..num_txs)
        .map(|_| {
            let fee_payer = fee_payers.next().unwrap();
            let program_id = Pubkey::new_unique();
            ...
            let serialized_tx = bincode::serialize(&versioned_transaction).unwrap();
            let serialized_tx_len = serialized_tx.len();
            bytes.extend_from_slice(&serialized_tx);
            let tx_bytes = bytes.split_to(serialized_tx_len);
            tx_bytes.freeze()
        })
        .collect();

    Arc::new(PacketBatchBytes {
        batches: txs
            .into_iter()
            .chunks(NUM_PACKETS)
            .into_iter()
...
trait NoCopyReceiveAndBufferCreator {
    fn create(
        receiver: Receiver<Arc<PacketBatchBytes>>,
        bank_forks: Arc<RwLock<BankForks>>,
    ) -> Self;
}

impl NoCopyReceiveAndBufferCreator for NoCopyTransactionViewReceiveAndBuffer {
    fn create(
        receiver: Receiver<Arc<PacketBatchBytes>>,
        bank_forks: Arc<RwLock<BankForks>>,
    ) -> Self {
        NoCopyTransactionViewReceiveAndBuffer {
            receiver,
            bank_forks,
        }
    }
...
fn bench_no_copy_receive_and_buffer<T: ReceiveAndBuffer + NoCopyReceiveAndBufferCreator>(
    c: &mut Criterion,
    bench_name: &str,
    num_instructions_per_tx: usize,
    probability_invalid_blockhash: f64,
    ...
    let txs = generate_no_copy_transactions(
        num_txs,
        bank.clone(),
        &fee_payers,
        num_instructions_per_tx,
...
fn bench_no_copy_view(c: &mut Criterion) {
    bench_no_copy_receive_and_buffer::<NoCopyTransactionViewReceiveAndBuffer>(
        c,
        "no_copy_view_max_instructions",
        MAX_INSTRUCTIONS_PER_TRANSACTION,
        0.0,
        true,
    );
