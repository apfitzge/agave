...
It joins a shaq queue as an untyped `SliceConsumer`, reads the matching `.wds`
wincode-dynamic schema at startup, decodes queue payloads through
`wincode_dynamic::Decoder`, and prints one JSON object per event to stdout.
...
`<events-dir>/bank_events` and `<events-dir>/bank_events.wds`. Use
`--queue-name <NAME>` for another queue. Use `--object <RECORD_NAME>` to print
only matching wincode-dynamic record variants.
